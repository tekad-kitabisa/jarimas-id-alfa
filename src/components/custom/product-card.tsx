...
export function ProductCard({ item, onSelect }: ProductCardProps) {
  ...
  const getWhatsAppLink = () => {
    ...
    const message = encodeURIComponent(
      `Halo Bpk/Ibu ${item.seller?.full_name || "Penjual"}, saya melihat produk Anda "${item.title}" di Lapak Warga JARIMAS-ID. Apakah masih tersedia?`
    )
      ...
      <div>
        ...
        <div className="relative h-44 w-full bg-muted/40 overflow-hidden flex items-center justify-center border-b border-border/60">
          {item.image_url ? (
            ...
            <div className={`h-full w-full flex flex-col items-center justify-center p-4 transition-transform duration-300 group-hover:scale-105 ${config.iconBg}`}>
              <CategoryIcon className="h-12 w-12 opacity-80 mb-1" />
              <span className="text-[11px] font-medium opacity-60">Foto Produk / Layanan</span>
            </div>
          )}

          {/* Badges on Top */}
          <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 flex-wrap">
            <Badge variant="outline" className={`text-[10px] font-medium px-2 py-0.5 backdrop-blur bg-background/90 ${config.badgeColor}`}>
          ...
          {item.condition && (
            <div className="absolute top-2.5 right-2.5">
              <Badge variant="secondary" className="text-[10px] font-medium px-2 py-0.5 backdrop-blur bg-background/90 shadow-xs">
                {item.condition === "BARU" ? "Baru" : item.condition === "BEKAS" ? "Seken" : "Jasa"}
              </Badge>
            </div>
          )}
        </div>
        ...
        <CardHeader className="pb-2 pt-3">
          ...
          <div className="flex items-baseline justify-between gap-2">
            <span className={`text-base sm:text-lg font-extrabold tracking-tight ${item.price === 0 ? "text-rose-600 dark:text-rose-400" : "text-primary font-mono"}`}>
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground pt-0.5">
            ...
            <span className="truncate">{item.location || item.kelurahan_code || "Kota Tegal"}</span>
          </div>
        </CardHeader>
        ...
        <CardContent className="py-1 space-y-2.5">
          <CardDescription className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">
            {item.description || "Hubungi penjual untuk informasi spesifikasi produk, ketersediaan stok, atau pemesanan langsung."}
          </CardDescription>
